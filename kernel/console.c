...
{
	while(*str) {
		write_color_ch(bg, font, *str++);
	}
}

void con_color_write(c_color_t bg, c_color_t font, char *str)
{
	write_color_str(bg, font, str);	
}

void con_write(char *str) 
{
	con_color_write(c_black, c_white, str);
}
...
static void con_clear() 
{
	...
	set_cursor(0, 0);
	while(size--) {
		write_color_ch(c_black, c_white, ' ');
	}
	set_cursor(0, 0);	
}
